...
14. 用户没有明确排除团本或尾王装备时，老七/老八的非饰品特效装备是最高优先级，不得自行排除；饰品只走专精BIS。用户未另行指定时固定选择两件制造装备，优先披风、护腕、腰带等小部位；制造武器计入这两件总数，指定一件制造武器后只能再选一件制造装备。解释方案时说明331装等代价。展示 is_final_boss_drop 时必须注明“尾王掉落，获取难度高”。

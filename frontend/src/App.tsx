...
type Item = {
  ...
  has_special_effect:boolean; is_crafted:boolean; is_tier?:boolean; catalyst_eligible:boolean;
  tier_acquisition_method?:"curio_exchange"|"catalyst_conversion"|null;
...
const toolProgress:Record<string,string> = {
  update_build_state:"正在保存当前装备、属性目标和限制",
  get_build_state:"正在读取当前配装状态",
  calculate_current_stats:"正在计算当前装备属性",
  search_items:"正在查找当前专精可用装备",
  optimize_current_loadout:"正在计算并比较配装方案",
};
...
function itemSourceLabel(item:Item) {
  const source=item.sources?.find(value=>value.instance_name_zh_cn)?.instance_name_zh_cn;
  if(source)return source;
  if(item.is_crafted)return "制造";
  if(item.is_tier||item.tier_acquisition_method==="curio_exchange")return "珍玩兑换";
  if(item.tier_acquisition_method==="catalyst_conversion")return "催化转化";
  return "来源待补充";
}
...
function GearEntry({slot,item,side,equipment,gems,onReplace,onRemove}:{slot:string;item?:Item;side:"left"|"right";equipment?:EquipmentState;gems:Map<number,Gem>;onReplace:()=>void;onRemove:()=>void}) {
  ...
  const craftedStats=(Object.entries(equipment?.crafted_secondary_stats||{}) as [StatKey,number][]).filter(([,value])=>value);
...
function BuilderPage({specs}:{specs:Spec[]}) {
  ...
  async function openPicker(slot:string,current?:Item){setPicker({slot,current});setCrafting(null);setPickerBusy(true);setCandidates([]);const [class_key,spec_key]=spec.split(".");const load=(item_level:number,source_type?:string)=>json<{items:Item[]}>(`${API}/loot/items?${new URLSearchParams({item_level:String(item_level),class_key,spec_key,slot_key:slot,limit:"200",...(source_type?{source_type}:{})})}`);const [drops,crafted,delves]=await Promise.all([load(334),load(331),load(321,"delve")]);setCandidates([...drops.items,...crafted.items.filter(value=>value.is_crafted),...delves.items]);setPickerBusy(false);refreshWowhead()}
  ...
  async function send(event:FormEvent){event.preventDefault();if(!input.trim()||!session||busy)return;const text=input.trim();setInput("");setBusy(true);setProgress("正在理解你的配装要求");setSolutions([]);setMessages(value=>[...value,{role:"user",text},{role:"assistant",text:""}]);try{const response=await fetch(`${API}/builder/sessions/${session.session_id}/messages/stream`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:text})});if(!response.ok||!response.body)throw new Error("对话请求失败");const reader=response.body.getReader();const decoder=new TextDecoder();let buffer="";while(true){const {done,value}=await reader.read();buffer+=decoder.decode(value||new Uint8Array(),{stream:!done});const blocks=buffer.split("\n\n");buffer=blocks.pop()||"";for(const block of blocks){const eventName=block.match(/^event: (.+)$/m)?.[1];const line=block.match(/^data: (.+)$/m)?.[1];if(!line)continue;const data=JSON.parse(line);if(eventName==="status")setProgress(data.message||"正在处理配装请求");if(eventName==="tool_start")setProgress(toolProgress[data.tool]||"正在处理配装步骤");if(eventName==="tool_result")setProgress(data.success?(data.tool==="optimize_current_loadout"?"方案计算完成，正在整理结果":"上一步已完成，正在继续处理"):"当前步骤未完成，正在整理结果");if(eventName==="proposal"&&data?.solutions){setProgress("配装方案已生成");setSolutions(data.solutions)}if(eventName==="state")setSession(current=>current?{...current,state:data}:current);if(eventName==="done")setMessages(list=>list.map((message,index)=>index===list.length-1?{...message,text:data.message||"处理完成"}:message));if(eventName==="error")throw new Error("Agent 暂时不可用")}if(done)break}}catch(error){setMessages(list=>list.map((message,index)=>index===list.length-1?{...message,text:error instanceof Error?error.message:"请求失败"}:message))}finally{setBusy(false);setProgress("");refreshConversations().catch(()=>{})}}

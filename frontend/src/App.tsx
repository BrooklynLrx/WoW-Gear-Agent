...
function BuilderPage({specs}:{specs:Spec[]}) {
  ...
  async function fetchItems(state:BuildState) {
    ...
    return placeWeapons(state.equipment.map(selected=>{const item=found.find(value=>value.item_id===selected.item_id&&value.item_level===selected.item_level);return item&&selected.equipped_slot?{...item,display_slot_key:selected.equipped_slot}:item}).filter(Boolean) as Item[]);
  }
  ...
  async function openPicker(slot:string,current?:Item){setPicker({slot,current});setPickerBusy(true);setCandidates([]);const [class_key,spec_key]=spec.split(".");const query=new URLSearchParams({item_level:"334",class_key,spec_key,slot_key:slot==="off_hand"?"weapon":slot,limit:"200"});const result=await json<{items:Item[]}>(`${API}/loot/items?${query}`);setCandidates(result.items.filter(value=>slot==="off_hand"?value.can_equip_off_hand:slot==="weapon"?itemSlot(value)==="weapon":true));setPickerBusy(false);refreshWowhead()}
  async function chooseItem(item:Item){if(!session||!picker)return;const replacingWeapon=picker.slot==="weapon"&&Boolean(item.weapon_type?.startsWith("2h_")||item.weapon_type?.startsWith("ranged_"));const replacingOffhand=picker.slot==="off_hand";const removed=items.filter(value=>value.item_id===picker.current?.item_id||(replacingWeapon&&["weapon","off_hand"].includes(itemSlot(value)))||(replacingOffhand&&itemSlot(value)==="weapon"&&Boolean(value.weapon_type?.startsWith("2h_")||value.weapon_type?.startsWith("ranged_"))));const removedIds=new Set(removed.map(value=>value.item_id));const equipment=session.state.equipment.filter(value=>!removedIds.has(value.item_id));equipment.push({item_id:item.item_id,item_level:item.item_level,gems:[],crafted_secondary_stats:{},equipped_slot:picker.slot==="off_hand"?"off_hand":picker.slot==="weapon"?"weapon":null});await patchEquipment(session,equipment,[...items.filter(value=>!removedIds.has(value.item_id)),{...item,display_slot_key:picker.slot}]);setPicker(null)}
  ...
  return <main className="workspace builder-page">
    ...
    {picker&&<Modal onClose={()=>setPicker(null)} title={`更换${slotNames[picker.slot]}`} wide><div className="picker-list">{pickerBusy?<p>正在读取当前专精可用装备…</p>:candidates.map(item=><button className="picker-item" key={item.item_id} onClick={()=>chooseItem(item)}><img src={item.icon_url||""} alt=""/><span><strong>{item.name_zh_cn}</strong><small>{item.item_level} · {itemStats(item)}</small><small>{item.sources[0]?.instance_name_zh_cn||"制造"}</small></span>{item.has_special_effect&&<em>特效</em>}</button>)}</div></Modal>}
  </main>;
}

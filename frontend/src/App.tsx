...
function BuilderPage({specs}:{specs:Spec[]}) {
  ...
  async function chooseItem(item:Item,craftedStats:Partial<Stats>={}){if(!session||!picker)return;if(item.customizable_secondaries&&!Object.keys(craftedStats).length){setCrafting({item,selected:[]});return}const replacingWeapon=picker.slot==="weapon"&&Boolean(item.weapon_type?.startsWith("2h_")||item.weapon_type?.startsWith("ranged_"));const replacingOffhand=picker.slot==="off_hand";const removed=items.filter(value=>value.item_id===picker.current?.item_id||(replacingWeapon&&["weapon","off_hand"].includes(itemSlot(value)))||(replacingOffhand&&itemSlot(value)==="weapon"&&Boolean(value.weapon_type?.startsWith("2h_")||value.weapon_type?.startsWith("ranged_"))));const removedIds=new Set(removed.map(value=>value.item_id));const equipment=session.state.equipment.filter(value=>!removedIds.has(value.item_id));equipment.push({item_id:item.item_id,item_level:item.item_level,gems:[],crafted_secondary_stats:craftedStats,equipped_slot:picker.slot==="off_hand"?"off_hand":picker.slot==="weapon"?"weapon":null});const displayed=withCraftedStats({...item,display_slot_key:picker.slot},craftedStats);await patchEquipment(session,equipment,[...items.filter(value=>!removedIds.has(value.item_id)),displayed]);setCrafting(null);setPicker(null)}
  ...
  return <main className="workspace builder-page">
    ...
    {picker&&<Modal onClose={()=>{setCrafting(null);setPicker(null)}} title={crafting?`选择「${crafting.item.name_zh_cn}」绿字`:`更换${slotNames[picker.slot]}`} wide>{crafting?<div className="craft-picker"><p>选择两项制造属性，每项 +{crafting.item.customizable_secondary_amounts?.[0]||0}</p><div>{(crafting.item.secondary_stat_choices||[]).map(key=>{const active=crafting.selected.includes(key);return <button className={active?"active":""} key={key} onClick={()=>setCrafting({...crafting,selected:active?crafting.selected.filter(value=>value!==key):crafting.selected.length<2?[...crafting.selected,key]:crafting.selected})}>{statNames[key]}</button>})}</div><footer><button onClick={()=>setCrafting(null)}>返回</button><button className="primary" disabled={crafting.selected.length!==2} onClick={()=>chooseItem(crafting.item,Object.fromEntries(crafting.selected.map((key,index)=>[key,crafting.item.customizable_secondary_amounts?.[index]||0])) as Partial<Stats>)}>确认属性</button></footer></div>:<div className="picker-list">{pickerBusy?<p>正在读取当前专精可用装备…</p>:candidates.map(item=><button className="picker-item" key={`${item.item_id}-${item.item_level}`} onClick={()=>chooseItem(item)}><img src={item.icon_url||""} alt=""/><span><strong>{item.name_zh_cn}</strong><small>{item.item_level} · {item.customizable_secondaries?"可自选两项绿字":itemStats(item)}</small><small>{item.sources[0]?.instance_name_zh_cn||"制造"}</small></span>{item.is_crafted?<em>制造</em>:item.has_special_effect&&<em>特效</em>}</button>)}</div>}</Modal>}
  </main>;
}
